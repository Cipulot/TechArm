...
Servo servo4;
Servo Clamp;
int servo1_pin = 46; //Base
int servo2_pin = 44; //Motor_1
int servo3_pin = 45; //Motor_2
int servo4_pin = 10; //Motor_3
int servo5_pin = 11; //Clamp
int torch_pin = 2;
...
void TFTBOOTSEQUENCE() {
	...
	//function that check the bmp file, with a sub-routine for the offset and other stuff...
...
int TFTNORMAL() {
	...
	//tft.drawRect(0, 160, 80, 26, WHITE);
	//tft.drawRect(0, 186, 80, 26, WHITE);
	//tft.drawRect(0, 212, 80, 26, WHITE);
	//Text for layout
...
void setup() {
	...
	//TFT Boot sequence 
	TFTBOOTSEQUENCE();
	pinMode(27, INPUT); //demo pin
	pinMode(23, INPUT); //input pin for bluetooth module state
	...
	TFTNORMAL();
	if (digitalRead(27) == HIGH) { //demo mode engaged
		do {
			learndata = SD.open("demo.txt", FILE_READ);
			for (int a = 0; a < 278; a++) { //write moves to servos for each line (fixed)
...
void loop() {
	...
	if (Serial1.available()) {
		...
		//LOGGED
			...
			for (int j = 0; j < lines; j++) { //write moves to servos for each line in the txt file
				receiveA = learndata.readStringUntil('A');
				receiveX = learndata.readStringUntil('X');
				receiveY = learndata.readStringUntil('Y');
				receiveZ = learndata.readStringUntil('Z');
